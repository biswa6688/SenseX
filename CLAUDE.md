...
- Model stack: faster-whisper distil-large-v3 (STT) + pyannote speaker-diarization-community-1 (diarization, VBxClustering — NOT speaker-diarization-3.1, see DECISIONS.md #13) + Qwen2.5-3B-Instruct GGUF Q4_K_M via llama-cpp-python (summary/sentiment/QA, one combined call) + Piper (TTS, subprocess). CPU-only, budgeted to fit 16GB RAM — see ARCHITECTURE.md RAM table.
...
ffmpeg is now on PATH (installed via `winget install Gyan.FFmpeg`) — but that's the **static** build (exe only, no DLLs). pyannote.audio 4.x uses torchcodec for audio decoding, which needs FFmpeg **shared** DLLs (avcodec-*.dll etc), not the static exe. Fixed by extracting a BtbN shared build to `C:\ffmpeg-shared\extracted\...\bin` and adding that dir to user PATH. If `import torchcodec` or diarization throws "Could not load libtorchcodec", check that dir is still on PATH and the process was restarted after the PATH change (existing shells/servers don't pick it up).

llama-cpp-python built from source successfully on this machine (no prebuilt wheel for Python 3.13 yet) — that took several minutes; don't assume it hung if it's slow.
